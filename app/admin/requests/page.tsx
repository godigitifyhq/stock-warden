    ...
    <div className="space-y-6 page-enter">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0 md:space-x-4">
        ...
        <div className="flex items-center space-x-2">
          <Filter size={16} className="text-[--ink-secondary]" />
          <select 
            value={statusFilter} 
...
function RequestDrawer({ request, onClose, onSuccess }: { request: any, onClose: () => void, onSuccess: () => void }) {
  ...
  const allocateMutation = useMutation({
    mutationFn: () => api.patch(`/admin/requests/${request.id}/allocate`, {
      allocations: Object.entries(allocations).map(([requestItemId, quantityAllocated]) => ({ requestItemId, quantityAllocated }))
    }),
    onSuccess: () => { setAllocSaved(true); setTimeout(() => setAllocSaved(false), 4000) },
    onError: (err: any) => {
      const msg = err.response?.data?.error?.message || 'Failed to save allocations'
      toast.error(msg)
    }
  })
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Request approved successfully'); onSuccess() },
  ...
  const isActionable = request.status === 'REQUESTED' || request.status === 'PENDING'
  ...
  const hasAllocErrors = Object.values(allocErrors).some(Boolean) ||
    Object.values(allocations).some((v) => !v || v < 1)
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      >
        ...
        <div className="p-6 space-y-8 flex-1">
          ...
          <div>
            ...
            <div className="bg-[--bg-subtle] p-4 rounded-md text-sm space-y-2">
              <p><span className="text-[--ink-secondary] w-24 inline-block">Name:</span> <span className="font-medium">{request.user?.name}</span></p>
              <p><span className="text-[--ink-secondary] w-24 inline-block">Department:</span> {request.user?.department}</p>
              <p><span className="text-[--ink-secondary] w-24 inline-block">Employee ID:</span> {request.user?.employeeId}</p>
              <p><span className="text-[--ink-secondary] w-24 inline-block">Email:</span> {request.user?.email}</p>
            </div>
          </div>
                    ...
                    return (
                      <tr key={ri.id}>
                        ...
                        <td className="px-4 py-3 text-right font-medium">
                          {ri.quantityReq} <span className="text-[--ink-secondary] font-normal">{ri.item?.unit}</span>
                          {alloc !== ri.quantityReq && (
                            <span className="ml-1 text-xs text-[--status-requested]">→ {alloc}</span>
                          )}
                        </td>
          ...
          {/* Adjust Quantities (actionable requests only) */}
          {isActionable && (
            <div>
                      ...
                      <tr key={ri.id}>
                        ...
                        <td className="px-4 py-2">
                          <input
                            ...
                            className={`w-20 px-2 py-1 border rounded text-sm text-center focus:outline-none focus:ring-1 focus:ring-black ${allocErrors[ri.id] ? 'border-red-400' : 'border-[--border-default]'}`}
                          />
                          {allocErrors[ri.id] && (
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-sm border border-[--border-default] rounded-md font-medium hover:bg-[--bg-subtle] disabled:opacity-50 transition-colors"
                >
          ...
          {!isActionable && (
            <div>
              ...
              <div className="flex items-center space-x-3">
                ...
                {request.status === 'APPROVED' && <a href={`/api/user/requests/${request.id}/invoice-download`} target="_blank" rel="noreferrer" className="text-sm text-black hover:underline">View Invoice</a>}
              </div>
            </div>
          )}
        </div>
        ...
        {/* Action Bar */}
        {isActionable && (
          <div className="p-6 border-t border-[--border-default] bg-white space-y-4 sticky bottom-0">
            <div>
            ...
            <div className="flex space-x-3">
              <button
                ...
                className="flex-1 flex items-center justify-center space-x-2 py-2 border border-red-200 bg-red-50 text-red-700 rounded-md font-medium hover:bg-red-100 transition-colors disabled:opacity-50"
              >
                <XCircle size={18} />
                Reject
              </button>
              <button
                ...
                disabled={isProcessing}
                className="flex-1 flex items-center justify-center space-x-2 py-2 bg-black text-white rounded-md font-medium hover:bg-[--accent-hover] transition-colors disabled:opacity-50"
              >
                <CheckCircle size={18} />
                Approve
              </button>
      ...
      <ConfirmModal
        ...
        description={
          confirmAction === 'approve'
            ? 'Approve this request and notify the requester?'
            : 'Reject this request. This will notify the requester.'
        }
